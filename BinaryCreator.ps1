...
.DESCRIPTION
    ...
    2. Reads runtime settings from settings.psd1 (user-editable, optional).
    ...
    Command-line parameters override settings.psd1 values.

.PARAMETER OutputPath
    Directory for the final .bin file. Overrides settings.psd1 OutputPath.
    Default: output\bin\ (from config.psd1).

.PARAMETER Parallelism
    Download thread count for haveibeenpwned-downloader.
    Overrides settings.psd1 Parallelism. Default: 64.
...
.PARAMETER KeepHashFile
    ...
    Overrides settings.psd1 KeepHashFile.
...
# -----------------------------------------------------------------------------
#  Load settings.psd1 (user-editable runtime settings)
# -----------------------------------------------------------------------------
...
if (Test-Path $settingsPath) {
    try {
        ...
        Write-Host "[WARN]  Failed to read settings.psd1: $_" -ForegroundColor Yellow
    }
}

# Merge: command-line params > settings.psd1 > defaults
