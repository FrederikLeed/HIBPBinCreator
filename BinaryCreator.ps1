...
.DESCRIPTION
    ...
    3. Converts the resulting text file to a packed binary:
       - Default: pypsirepacker (Python, streaming, near-zero memory)
       - Legacy:  PsiRepacker.exe (C++, ~50 GB RAM)
    4. Final binary is saved as  hibpntlmhashes<ddMMyy>.bin  in output\bin\.

.PARAMETER Parallelism
    Download thread count for haveibeenpwned-downloader. Default: 64.
...
.PARAMETER UsePsiRepacker
    Use legacy PsiRepacker.exe instead of Python pypsirepacker.

.PARAMETER PsiRepackerPath
    Path to an existing PsiRepacker.exe (implies -UsePsiRepacker).
...
# -----------------------------------------------------------------------------
#  Determine repacker mode
# -----------------------------------------------------------------------------
$repackerMode = $cfg.RepackerMode

# Command-line overrides
if ($UsePsiRepacker -or ($PsiRepackerPath -ne '')) {
    $repackerMode = 'Legacy'
}

# Resolve repacker binary/paths
$pythonExe         = $cfg.PythonExe
$pyPsiRepackerDir  = $cfg.PyPsiRepackerDir
$psiRepackerExe    = $cfg.PsiRepackerExe

if ($PsiRepackerPath -ne '') {
    $psiRepackerExe = $PsiRepackerPath
}
...
if ($repackerMode -eq 'Legacy') {
    if (-not (Test-Path $psiRepackerExe)) {
        Write-Log "PsiRepacker.exe not found at: $psiRepackerExe" -Level ERROR
        Write-Log 'Specify a valid path via -PsiRepackerPath or re-run PrepareEnv.ps1.' -Level ERROR
        exit 1
    }
    Write-Log "PsiRepacker.exe           : $psiRepackerExe" -Level SUCCESS
    Write-Log 'Repacker mode             : Legacy (PsiRepacker.exe)' -Level INFO
} else {
    # Validate Python
    ...
    Write-Log 'Repacker mode             : Python (pypsirepacker)' -Level INFO
}
...
if ($repackerMode -eq 'Legacy') {
    # -------------------------------------------------------------------------
    #  Legacy: PsiRepacker.exe with spinner
    # -------------------------------------------------------------------------
    ...
    foreach ($line in $stdoutLines) {
        $ts = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
        Add-Content -Path $script:LogFile -Value "[$ts] [PSI]    $line"
        if ($line.Trim()) { Write-Host "  $line" -ForegroundColor DarkGray }
    }
    foreach ($line in $stderrLines) {
        $ts = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
        Add-Content -Path $script:LogFile -Value "[$ts] [PSI-ERR] $line"
    ...
    # -------------------------------------------------------------------------
    #  Default: Python pypsirepacker (streaming, near-zero memory)
    # -------------------------------------------------------------------------
