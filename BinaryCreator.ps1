...
.DESCRIPTION
    1. Reads config.psd1 (generated by PrepareEnv.ps1) for environment paths
       and user settings (OutputPath, Parallelism, etc.).
    2. Downloads all NTLM hashes with haveibeenpwned-downloader.
    3. Converts the resulting text file to a packed binary using pypsirepacker.
    4. Final binary is saved as  hibpntlmhashes<ddMMyy>.bin  in OutputPath.

    Command-line parameters override config.psd1 values.

.PARAMETER OutputPath
    Directory for the final .bin file. Overrides config.psd1 OutputPath.
    Default: output\bin\ (from config.psd1).

.PARAMETER Parallelism
    Download thread count for haveibeenpwned-downloader.
    Overrides config.psd1 Parallelism. Default: 64.
...
.PARAMETER KeepHashFile
    ...
    Overrides config.psd1 KeepHashFile.
