...
# Preserve user settings from existing config (OutputPath, Parallelism, etc.)
$userKeys = @('OutputPath', 'Parallelism', 'KeepHashFile', 'NoOverwrite', 'SkipDownload')
$existingUser = @{}
if (Test-Path $configPath) {
    try {
        $prev = Import-PowerShellDataFile $configPath
        foreach ($key in $userKeys) {
            if ($prev.ContainsKey($key)) {
                $existingUser[$key] = $prev[$key]
            }
        }
    } catch {
        Write-Log "Could not read existing config.psd1: $_" -Level WARN
...
# Merge: keep existing user values, fill defaults for missing keys
$outputPathVal  = if ($existingUser.ContainsKey('OutputPath'))   { $existingUser.OutputPath -replace "'","''" } else { '' }
$parallelismVal = if ($existingUser.ContainsKey('Parallelism'))  { $existingUser.Parallelism } else { 64 }
$keepHashVal    = if ($existingUser.ContainsKey('KeepHashFile')) { if ($existingUser.KeepHashFile) { '$true' } else { '$false' } } else { '$false' }
$noOverwriteVal = if ($existingUser.ContainsKey('NoOverwrite'))  { if ($existingUser.NoOverwrite)  { '$true' } else { '$false' } } else { '$false' }
$skipDlVal      = if ($existingUser.ContainsKey('SkipDownload')) { if ($existingUser.SkipDownload) { '$true' } else { '$false' } } else { '$false' }

$config = @"
# Generated by PrepareEnv.ps1 on $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
# Re-run PrepareEnv.ps1 to refresh environment paths.
# Edit the Settings section below to customise runtime defaults.
@{
    # ---- Environment (auto-detected, do not edit) ----
    ...
    KeepHashFile     = $keepHashVal
    NoOverwrite      = $noOverwriteVal
    SkipDownload     = $skipDlVal
}
"@
