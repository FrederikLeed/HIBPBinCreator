...
### PrepareEnv.ps1
Environment bootstrapper. Validates prerequisites, installs missing tools, writes `config.psd1`.

### BinaryCreator.ps1
Main pipeline script. Reads `config.psd1`, downloads hashes, converts to binary, validates output.
...
| `RepackerMode` | string | `Python` (default) or `Legacy` |
| `PythonExe` | string | Path to Python executable |
| `PyPsiRepackerDir` | string | Path to pypsirepacker package directory |
| `PsiRepackerExe` | string | Path to PsiRepacker.exe (legacy mode only) |

## Repacker Comparison

| Feature | pypsirepacker (Python) | PsiRepacker.exe (C++) |
| --- | --- | --- |
| Memory usage | Near-zero (streaming) | ~50 GB (loads entire file) |
| Platform | Cross-platform | Windows only |
| Dependencies | Python 3.6+ (stdlib only) | Visual Studio / MSBuild |
| Speed | Comparable | Comparable |
| Sort verification | Built-in | None |
| Default | Yes | No (opt-in via `-UsePsiRepacker`) |
